...
export class NotionPushListener {
  ...
  async handleCompanyCreated(
    ...
    this.logger.log(`Handling company created event for workspace ${payload.workspaceId}`);
  ...
  async handleCompanyUpdated(
    ...
    this.logger.log(`Handling company updated event for workspace ${payload.workspaceId}`);
    for (const event of payload.events) {
      await this.notionSyncService.createOrUpdateCompanyInNotion(
        payload.workspaceId,
        event.properties.after,
  ...
  async handlePersonCreated(
    ...
    this.logger.log(`Handling person created event for workspace ${payload.workspaceId}`);
    for (const event of payload.events) {
  ...
  async handlePersonUpdated(
    ...
    this.logger.log(`Handling person updated event for workspace ${payload.workspaceId}`);
    for (const event of payload.events) {
      await this.notionSyncService.createOrUpdatePersonInNotion(
        payload.workspaceId,
        event.properties.after,
